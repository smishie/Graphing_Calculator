...
Sidebar::Sidebar(float left, float width, vector<string>& old_equations) : _left(left), _width(width)
{
    cout << "Sidebar CTOR: TOP" << endl;
    items.reserve(50);

    // set up the sidebar rectangle:
    rect.setFillColor(sf::Color(230, 230, 250)); //(192,192,192)); //silver
    ...
    sb_text.setFillColor(sf::Color(170, 51, 106));
    ...
    //     f >> old_equations[i];
    //     history.push_back(old_equations[i]);
    // }
    // f.close();
...
void Sidebar::draw(sf::RenderWindow &window)
{
         ...
         it != items.end(); it++)
    {
        ...
        height += sb_text.getLocalBounds().height + VERTICAL_LINE_SPACING / 2;
        if (!blank)
            window.draw(sb_text);
    }
    // if(!history.empty()){
